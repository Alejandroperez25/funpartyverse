  ...
  return <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative rounded-full h-10 w-10 mx-0 py-[13px] px-[28px] bg-orange-500 hover:bg-orange-400">
          ...
          {totalItems > 0 && <span className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center rounded-full bg-funneepurple text-white text-xs">
              {totalItems}
            </span>}
        </Button>
      </SheetTrigger>
        ...
        <div className="mt-8 flex flex-col h-[calc(100vh-13rem)]">
          {items.length === 0 ? <div className="flex flex-col items-center justify-center h-full">
              ...
              <div className="flex-1 overflow-auto space-y-4 pr-2">
                {items.map(item => <CartItemCard key={item.id} item={item} onRemove={() => removeFromCart(item.id)} onUpdateQuantity={quantity => updateQuantity(item.id, quantity)} />)}
              </div>
              ...
              <div className="border-t pt-4 mt-4 space-y-4">
                ...
                <Button className="w-full bg-funneepurple hover:bg-funneepurple/90 text-white" onClick={handleCheckout}>
                  {t('cart.checkout')}
                </Button>
              </div>
            </>}
  ...
  return <motion.div initial={{
    ...
    y: -10
      ...
      <div className="flex items-center">
        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => onUpdateQuantity(item.quantity - 1)}>
